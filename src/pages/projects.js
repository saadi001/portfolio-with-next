...
const FeaturedProduct = ({ type, title, summary, img, link, github }) => {
  return (
    <article className="w-full flex justify-between items-center rounded-br-2xl rounded-3xl border border-solid border-dark bg-light shadow-2x p-12 relative">
      <div className="absolute top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2.5rem] bg-dark rounded-br-2xl"></div>
      <Link
        href={link}
        target="_blank"
    ...
    <>
      ...
      <div className="absolute top-0 -right-3 -z-10 w-[101%] h-[103%] rounded-[2rem] bg-dark rounded-br-2xl"></div>
        ...
        <div className="w-full flex flex-col items-start justify-between mt-4">
          <span className="text-primary font-medium text-xl text-left">
            {type}
          </span>
          <Link
            href={link}
